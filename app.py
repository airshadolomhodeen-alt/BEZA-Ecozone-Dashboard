...
def load_datasets():
    ...
    # 3. Sources Master Dataset (sources.csv)
...
if app_page == "🌍 Executive Summary & Spatial Map":
    ...
    # Interactive PyDeck Map with exact geographical coordinates
    ...
    if len(filtered_zones) > 0:
        def get_color(status):
            if status == "Operating":
                return [52, 211, 153, 200]  # Emerald green
            elif status == "Non-Operating":
                return [251, 191, 36, 200]  # Amber
            else:
